...
<% end_if %>
<% if canPost %>
	<p><a href="{$Link}starttopic" title="<% _t('Forum_ss.NEWTOPIC','Click here to start a new topic') %>"><img src="silverstripe/silverstripe-forum:client/images/forum_startTopic.gif" alt="<% _t('Forum_ss.NEWTOPICIMAGE','Start new topic') %>" /></a></p>
<% end_if %>
	...
	<% if $getStickyTopics(0) %>
		<table class="forum-sticky-topics" class="topicList" summary="List of sticky topics in this forum">
			...
			<% loop $getStickyTopics(0) %>
				<% include SilverStripe\\Forum\\TopicListing %>
			<% end_loop %>
		</table>
	<% end_if %>
