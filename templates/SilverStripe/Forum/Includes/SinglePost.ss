<div id="post{$ID}" class="forum-post">
	...
	<div class="user-content">
		...
		<div class="quick-reply">
			<% if $Thread.canPost %>
				<p>$Top.ReplyLink.RAW</p>
			<% end_if %>
		</div>
		<h4><a href="$Link">$Title <img src="silverstripe/silverstripe-forum:client/images/right.png" alt="Link to this post" title="Link to this post" /></a></h4>
		...
		<% if $EditLink || $DeleteLink %>
			<div class="post-modifiers">
				...
				<% if $BanLink || $GhostLink %>
					|
					<% if $BanLink %>$BanLink.RAW<% end_if %>
					<% if $GhostLink %>$GhostLink.RAW<% end_if %>
				<% end_if %>
